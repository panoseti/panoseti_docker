...
sudo docker run -itd --name panoseti-dns --hostname panoseti-dns -v /var/run/docker.sock:/var/run/docker.sock -v /etc/resolv.conf:/etc/resolv.conf defreitas/dns-proxy-server
